...
- **Localmaxing** (Dual RTX): the second-card recipe — **one** isolated `llama-server` per GPU so both 96 GB cards generate at once. Pick by what the second stream needs (another 27B, fast Coder, or fast 35B-A3B). Two processes on one GPU queue; leftover VRAM is empty weight storage, not idle decode. Guide: [Dual-RTX6000-Qwen3.8-localmaxing.md](Dual-RTX6000-192GB/Dual-RTX6000-Qwen3.8-localmaxing.md).
...
- **Thinking / reasoning (MiniCPM5-2B)**: OpenBMB documents Think / No-think via **`--reasoning on|off`** (unlike Muse / LFM). That toggle is **untested** on this repo’s Jetson build — confirm with the guide’s curl. Do not use `--chat-template-kwargs '{"enable_thinking":…}'`. Published 2B mode is Think (temp 1.0 / top_p 0.95); Jetson PRIMARY **forces off** for Pi tools. See [agentic harnesses — MiniCPM5](agentic-harnesses.md#minicpm5-2b--pi-coding-agent).
- **DFlash**: Muse Glimmer’s speculative-decode companion (~1.6 GB GGUF). Block-diffusion draft (`--spec-type draft-dflash`).
...
- **Qwen3-Coder-Next (released 2026-02)**: Qwen coding MoE, **80B total / 3B active** (GGUF arch `qwen3next`), native 262k, **non-thinking**. Dual RTX guide ⚠️ untested: [Dual-RTX6000-Qwen3-Coder-Next.md](Dual-RTX6000-192GB/Dual-RTX6000-Qwen3-Coder-Next.md) — download **UD-Q6_K_XL** only (~73 GB, sharded) @ 262k on one 96 GB card. Official sampling temp **1.0** / top_k **40** / min_p **0.01**. Not a drop-in for the dense 27B Q8 host.
- **Ternary Bonsai 2 27B (released 2026-09)**: PrismML ternary pack of Qwen3.8-27B (`{−1,0,+1}` weights + FP16 group scales, rotated basis). Two GGUF packings: **`PQ2_0`** (~7.2 GB, cheaper unpack) and **`PTQ1_0`** (~5.9 GB, denser trits). Guides ⚠️ untested: [Dual RTX](Dual-RTX6000-192GB/Dual-RTX6000-Bonsai-2-27B.md) (CUDA, 262k) · [DGX Spark](DGX-Spark-128GB/DGX-Spark-Bonsai-2-27B.md) (CUDA GB10, 262k) · [M1 Ultra Studio](M1-Ultra-Studio-64GB/M1-Ultra-Studio-Bonsai-2-27B.md) (Metal, 262k) · [RTX 3090 WSL2](Win-RTX3090-24GB/Windows-RTX3090-Bonsai-2-27B.md) (CUDA, 131k) · [7900 XTX](AMD-7900-XTX/7900-XTX-Bonsai-2-27B.md) (HIP, 131k). Needs [PrismML-Eng/llama.cpp](https://github.com/PrismML-Eng/llama.cpp) (`prism-b10658+`). Stock llama.cpp / turboquant refuse the types. Vulkan `PQ2_0` currently falls back to CPU.
...
- **MiniCPM5-2B (released September 2026)**: OpenBMB dense ~2.52B Llama (Apache 2.0), native 131k context, **text-only**. Official GGUF: [openbmb/MiniCPM5-2B-GGUF](https://huggingface.co/openbmb/MiniCPM5-2B-GGUF). Jetson guide ⚠️ untested: [Jetson-Orin-MiniCPM5-2B.md](Jetson-Orin-Nano-Super/Jetson-Orin-MiniCPM5-2B.md). Vision lives in the separate MiniCPM-V line, not this checkpoint.
...
- [Qwen3-Coder-Next — How to Run Locally (Unsloth)](https://unsloth.ai/docs/models/qwen3-coder-next): sampling, hardware table, llama.cpp
- [unsloth/Qwen3-Coder-Next-GGUF](https://huggingface.co/unsloth/Qwen3-Coder-Next-GGUF): GGUF catalog (Dual RTX pulls `UD-Q6_K_XL` only; confirm filenames before download)
- [prism-ml/Ternary-Bonsai-2-27B-gguf](https://huggingface.co/prism-ml/Ternary-Bonsai-2-27B-gguf): ternary GGUF packs (`PQ2_0` / `PTQ1_0`)
- [Ternary Bonsai 2 27B (PrismML docs)](https://docs.prismml.com/bonsai-2-27b): specs, sampling, fork requirement
...
- [openbmb/MiniCPM5-2B-GGUF](https://huggingface.co/openbmb/MiniCPM5-2B-GGUF): official GGUF (Q8_0 / Q4_K_M / F16)
- [Deploy MiniCPM5 with llama.cpp (OpenBMB)](https://github.com/OpenBMB/MiniCPM/blob/main/docs/deployment/llama_cpp.md): sampling table, `-c 8192` example, `min_p=0.0` note
