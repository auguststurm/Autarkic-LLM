...
- **Checkpointing / cache-ram** can help non-hybrid models; **hybrid Qwen (3.5/3.6/3.8)** guides usually omit checkpoints and may use `--cache-ram 0` for multi-turn correctness (§5). The LFM2.5 Jetson and RTX 3090 guides omit them too (hybrid conv + GQA).
...
- **`-DGGML_CUDA_FA_ALL_QUANTS=ON`**: compile flash-attention CUDA kernels for **all** KV-quant type combinations. Needed when you want arbitrary quantized KV (e.g. turbo/odd combos) with flash attention; **significantly longer compile time**. Used on Jetson and RTX 4090. **Do not use on RTX 3090** (Ampere D=512 turbo FA vec).
