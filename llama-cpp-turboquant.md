...
- **Pi + LFM2.5-2.6B:** omit `--reasoning off` (template always opens `<think>`). Official sampling **temp 0.1 / top_k 50 / repeat-penalty 1.1**. Jetson primary **Q8_0 @ 64k** (✅ tested; native stretch **128k**), `--n-predict` / Pi `maxTokens` **8192** (thinking counts against the cap). Keep q8/q8 — KV is cheap on this hybrid. See [Jetson LFM2.5](Jetson-Orin-Nano-Super/Jetson-Orin-LFM2.5-2.6B.md).
