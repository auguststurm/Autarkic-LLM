...
- **CUDA — room / agent quality (Win 4090 field-validated Pi baseline; prefer when VRAM allows):** `--cache-type-k q8_0 --cache-type-v q8_0`, then raise `--ctx-size`; add **turbo V** only if overflow forces capacity. See [Windows RTX 4090](Win-RTX4090-24GB/Windows-RTX4090-Qwen3.6.md). LFM2.5 on the 3090 stays q8/q8 at native **128k** (KV ~1.1 Gi; ⚠️ untested): [Windows RTX 3090 LFM2.5](Win-RTX3090-24GB/Windows-RTX3090-LFM2.5-2.6B.md).
...
- **`-DCMAKE_CUDA_ARCHITECTURES="…"`**: compile for a specific GPU compute capability. Common values: RTX 3090 `"86"`, Jetson Orin `"87"`, RTX 4090 `"89"`, RTX 6000 Pro Max-Q `"120"`, DGX Spark GB10 `"121"`. Omit to let CMake autodetect on the build machine.
- **`-DGGML_CUDA_F16=ON`**: enable FP16 CUDA compute paths (faster on capable GPUs).
- **`-DGGML_CUDA_FA_ALL_QUANTS=ON`**: compile flash-attention CUDA kernels for **all** KV-quant type combinations. Needed when you want arbitrary quantized KV (e.g. turbo/odd combos) with flash attention; **significantly longer compile time**. Used in the Jetson and RTX 3090 LFM guides.
