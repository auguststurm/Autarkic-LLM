...
Hardware-specific numbers always come from **your** guide (pin table + `models.json`). Guide *shape* (download → cmake → PRIMARY → JSON): [Dual RTX Qwen3.8](Dual-RTX6000-192GB/Dual-RTX6000-Qwen3.8.md). Tight Metal / `--fit` crush / turbo2 V: [M4 Air](M4-MacBook-Air-24GB/M4-MacBook-Air-Qwen3.6.md). Field-validated **Pi + dense Qwen 27B** lessons below cover **Qwen3.6-27B** and **Qwen3.8-27B** (Dual RTX 3.8 is field-tested). Dual RTX two cards (one model each; pick by the second stream): [Localmaxing](Dual-RTX6000-192GB/Dual-RTX6000-Qwen3.8-localmaxing.md). **Qwen3-Coder-Next** (80B-A3B, one Dual RTX card, ⚠️ untested): [guide](Dual-RTX6000-192GB/Dual-RTX6000-Qwen3-Coder-Next.md) — two-limit / no-DRY / `--fit off` still apply; use that page’s **temp 1.0 / top_k 40 / min_p 0.01** sampler, not the dense-27B Pi row. Muse Glimmer Pi pin: [Muse Glimmer 30B + Pi](#muse-glimmer-30b--pi-coding-agent). LFM2.5-2.6B Pi pin: [LFM2.5-2.6B + Pi](#lfm25-26b--pi-coding-agent). MiniCPM5-2B Pi pin: [MiniCPM5-2B + Pi](#minicpm5-2b--pi-coding-agent). Ternary Bonsai 2 is the same Qwen3.8 backbone on a **different binary** ([Dual RTX Bonsai 2](Dual-RTX6000-192GB/Dual-RTX6000-Bonsai-2-27B.md) · [DGX Spark](DGX-Spark-128GB/DGX-Spark-Bonsai-2-27B.md) · [M1 Ultra Studio](M1-Ultra-Studio-64GB/M1-Ultra-Studio-Bonsai-2-27B.md) · [RTX 3090 WSL2](Win-RTX3090-24GB/Windows-RTX3090-Bonsai-2-27B.md) · [7900 XTX HIP](AMD-7900-XTX/7900-XTX-Bonsai-2-27B.md)) — Pi JSON shape matches, engine does not.

## Qwen3.6-27B + Pi Coding Agent (cross-hardware)

> **Also applies to Qwen3.8-27B.** Same hybrid thinking, tool-loop risks, and two token limits. **✅ Field-tested on Dual RTX 6000** ([guide](Dual-RTX6000-192GB/Dual-RTX6000-Qwen3.8.md), 2026-08-14). Other 3.8 ports still ⚠️ untested: [DGX Spark](DGX-Spark-128GB/DGX-Spark-Qwen3.8.md) · [M5 Pro](M5-MacBook-Pro-48GB/M5-MacBook-Pro-Qwen3.8.md). Overview: [README — Qwen3.8](README.md#qwen38-2026-08-14). **Ternary Bonsai 2 27B** is that same backbone, ternary-packed — use the same Pi two-limit / no-DRY / `--reasoning off` lessons, but serve it from the PrismML fork ([guide](Dual-RTX6000-192GB/Dual-RTX6000-Bonsai-2-27B.md), ⚠️ untested).
...
| **`--ctx-size` / `contextWindow` 65536** on 8 GB (stretch **131072**, FA **on**); **131072 / FA off** on 24 GB Ampere WSL; **131072 / FA on** on DGX Spark GB10 | Gemma’s 16k window (Pi first-turn `length`; [guide](Jetson-Orin-Nano-Super/Jetson-Orin-LFM2.5-2.6B.md#pi-truncation-on-the-first-turn)); 4090 `FA_ALL_QUANTS` cmake; 3090 `FA=OFF` cmake on Spark |
| **`--n-predict` / `maxTokens` 8192** on the Jetson (raise both to **16384** if still `length`); **16384** on the 3090 and Spark — think tokens still count against the cap | `maxTokens` 4096 |
