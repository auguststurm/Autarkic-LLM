...
**LFM2.5-2.6B guide:** [Jetson Orin Nano Super](Jetson-Orin-Nano-Super/Jetson-Orin-LFM2.5-2.6B.md) ⚠️ untested (2026-09-07). Official Liquid GGUF. **Q8_0 @ 32k q8/q8**, `--n-predict` / Pi `maxTokens` **8192** (thinking counts against the cap; 16k / 4096 truncates on the first Pi turn). **Q6_K** (2.22 GB) is the headroom swap. Official sampling **temp 0.1 / top_k 50 / repeat-penalty 1.1**. Template always opens `<think>` — omit `--reasoning off`. Pi JSON: `reasoning` true, `thinkingLevelMap.off` null. Docs: [Liquid model card](https://huggingface.co/LiquidAI/LFM2.5-2.6B).
