...
- **RTX 3090 (sm_86) is not the 4090 cmake.** Ada (`"89"` + `FA_ALL_QUANTS`) compiles D=512 TurboQuant flash-attn vec kernels; Ampere `ptxas` rejects them (`0x10100` vs `0xc000` shared). 3090 turboquant pin: [Windows-RTX3090-LFM2.5-2.6B.md](Win-RTX3090-24GB/Windows-RTX3090-LFM2.5-2.6B.md) (`FA=OFF`, `86-real`, `NO_VMM`, graphs off).
...
**LFM2.5-2.6B guides:** [Jetson Orin Nano Super](Jetson-Orin-Nano-Super/Jetson-Orin-LFM2.5-2.6B.md) **✅ Tested** (2026-09-08, Pi) — **Q8_0 @ 64k q8/q8**, `--n-predict` / Pi `maxTokens` **8192** (native stretch **128k**). [Windows RTX 3090 WSL2](Win-RTX3090-24GB/Windows-RTX3090-LFM2.5-2.6B.md) ⚠️ untested (ported 2026-09-20) — **Q8_0 @ 128k q8/q8 / FA off**, `--n-predict` / Pi `maxTokens` **16384** (`~/AIML` / `~/GitHub`; cmake `86-real`, not the 4090 `FA_ALL_QUANTS`). [DGX Spark](DGX-Spark-128GB/DGX-Spark-LFM2.5-2.6B.md) ⚠️ untested (ported 2026-09-20) — **Q8_0 @ 128k q8/q8 / FA on**, `--n-predict` / Pi `maxTokens` **16384** (this folder’s GB10 `"121"` cmake, **not** Jetson `FA_ALL_QUANTS` or 3090 `FA=OFF`). Official Liquid GGUF. Think tokens count against the cap; 16k / 4096 truncates on the first Pi turn. **Q6_K** (2.22 GB) is the Jetson headroom swap (skip on 24 GB / 128 GB). Optional **F16** (5.4 GB) on roomy CUDA. Official sampling **temp 0.1 / top_k 50 / repeat-penalty 1.1**. Template always opens `<think>` — omit server `--reasoning off`. Pi **skills/tools:** `reasoning` false, no `thinkingLevelMap`. Pi **traces:** `reasoning` true, `thinkingLevelMap.off` null. Docs: [Liquid model card](https://huggingface.co/LiquidAI/LFM2.5-2.6B).
